...
var searchAutocomplete = {
  init: function(){
    ...
    var currentAjaxRequest = null;
      ...
      var searchPath = '/search?type=product&q=';
            ...
            if(data.results_count == 0) {
              ...
              $.each(data.results, function(index, item) {
                ...
                if(item.object_type == 'product') {
                  ...
                  link.append('<div class="title">' + item.title + '<br><span class="item-pricing">'+ item.price +'</span></div>');
  ...
  var quickShop = {
    ...
    updateVariant: function(variant){
      ...
      if (globalQuickShopProduct != 'undefined'){
        ...
        //Title and Vendor
        $('.js-product-title').html(product.ly_title);
        $('.js-product-vendor')
        .html('<a href="/collections/vendors?q=' + product.vendor +'">' + product.vendor + '</a>');

        //Product Description
        $('.js-full-description').html(product.full_description);
        $('.js-regular-description').html(product.regular_description.split('*****')[2]);  //.substring added to remove comment spilter from quick view
      ...
      $.each(cart.items, function(index, item) {
        ...
        if(item.properties) {
          $.each(item.properties, function(title, value) {
            if (title == "_title") {
              var tempTitle = value;
              cart_items_html += '<span class="money">' + Shopify.formatMoney(item.price, $('body').data('money-format')) + '</span></strong>' + '<div class="item_title">' + tempTitle;
